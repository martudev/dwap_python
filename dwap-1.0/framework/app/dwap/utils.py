...
dwap_env['origin_path'] = os.path.join(
    os.path.dirname(os.path.realpath(sys.argv[0])))
dwap_env['path_to_modules'] = os.path.join(os.path.dirname(
    os.path.realpath(sys.argv[0])), 'framework', 'app', 'dwap', 'modules')
...
def printEnviroment():
    ...
    if enviroment == 'test':
        ...
        print(
            "~  Usage: dwap <cmd> or dwap <cmd> --[options]=<value>.                ")
        print("~                                                                      ")
        print("~  Commands <cmd>:                                                     ")
        print("~ -------------------------------------------------------------------- ")
        ...
        print(
            "~  * <cmd> --[options]=<value>:   with <value>,  x_value     Example: dwap test --test_wpp=true")
        ...
        print(
            "~  Usage: dwap <cmd> --options [options, ...].                         ")
        print("~                                                                      ")
...
def getChromeVersion():
    chrome_driver = os.path.join(
        dwap_env['path_to_modules'], 'chromedriver_74')
    chrome_version_options = Options()
    # lo hacemos para minimizar el chrome ya que solo queremos obtener la version
    chrome_version_options.add_argument("--headless")
    versionDriver = webdriver.Chrome(
        options=chrome_version_options, executable_path=chrome_driver)
    # 'chromedriver_74' => 'version' | 'chromedriver_75' => 'browserVersion'
